...
func TestAuthorizationService_GetAuthorizedUser(t *testing.T) {
	...
	artifacts, _, err := client.Authorization.GetAuthorizedUser(ctx)
	if err != nil {
		t.Errorf("Actions.ListArtifacts returned error: %v", err)
	}

	want := &User{ID: 123, Username: "John Doe", Color: "#000000", ProfilePicture: "https://clickup.com/avatar.jpg"}
	if !cmp.Equal(artifacts, want) {
		t.Errorf("Actions.ListArtifacts returned %+v, want %+v", artifacts, want)
	}
}

func TestAuthorizationService_GetAuthorizedTeams(t *testing.T) {
	client, mux, _, teardown := setup()
	defer teardown()
